...
server.use((0, _cors["default"])()).use(_express["default"]["static"]((0, _path.join)(_dirname, '../public'))) // Servir arquivos estáticos da pasta 'public'
.use(_express["default"].urlencoded({
  extended: true
})).set('views', (0, _path.join)(_dirname, './views')) // Definir o diretório 'views'
.set('view engine', 'hbs').get('/', _pages.index).get('/orphanages', _pages.orphanages).get('/orphanage', _pages.orphanage).get('/create-orphanage', _pages.createOrphanage).post('/save-orphanage', _pages.saveOrphanage);
